...
func TestPlatform(t *testing.T) {
	if got := New().Platform(); got != connector.PlatformLinkedIn {
		t.Fatalf("Platform() = %q, want %q", got, connector.PlatformLinkedIn)
	}
}
...
func TestCapabilitiesReturnsAFreshSlice(t *testing.T) {
	...
	first[0] = connector.CapabilityComments
	if c.Capabilities()[0] == connector.CapabilityComments {
